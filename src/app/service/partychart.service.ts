...
export class PartychartService {
  ...
  addChatMessage(msg, id) {
    return this.afs.collection('partychats').doc(id).collection('messages').add({
      msg: msg,
  ...
  getChatMessages(id) {
    ...
    return this.getUsers().pipe(
      switchMap(res => {
        users = res;
        return this.afs.collection('partychats').doc(id).collection('messages', ref => ref.orderBy('createdAt')).valueChanges({ idField: 'id' }) as Observable<Message[]>;
      }),
      map(messages => {
