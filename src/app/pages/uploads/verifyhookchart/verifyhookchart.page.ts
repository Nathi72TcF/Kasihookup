...
@Component({
  selector: 'app-verifyhookchart',
  templateUrl: './verifyhookchart.page.html',
  styleUrls: ['./verifyhookchart.page.scss'],
})
export class VerifyhookchartPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.postID = this.id;
      this.postReference = this.afs.doc(`hookupgirls/${this.postID}`);
  ...
  upDateLikes() {
    // like
    this.db.collection('hookupgirls').doc(this.id).update({
      like: this.likesNo,
    })
  }
  ...
  getFeedPost(id) {
    this.db.collection('hookupgirls').doc(id)
    .onSnapshot(snapshot => {
  ...
  updateCommentNo(id) {
    this.db.collection('hookupgirls').doc(id).update({ comments: this.commentsNo })
  }
